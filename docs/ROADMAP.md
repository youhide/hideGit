...
| Area | Notes |
|---|---|
| **Submodules** | Status, update, init. Common enough to matter, awkward enough to deserve its own milestone. **Read and shown**: `GitBackend::submodules` reports each entry's path, URL, the commit the superproject records and the commit the nested checkout is actually on — the three states `git submodule status` prints as `-`, ` ` and `+` — and the sidebar has a `SUBMODULES` section that names both commits when they disagree. `GitBackend::update_submodules` is the write half — `git submodule update`, with `--init` for the ones that were never set up — and it answers with the submodules **as they are afterwards** rather than with `()`, because `git submodule update` without `--init` exits 0 having done nothing. The sidebar row calls it: a submodule with no checkout offers to set one up, one that moved offers to go back, and one already at the recorded commit offers nothing because there is nothing an update would change. |
| **Worktrees** | Growing in use; fits naturally alongside multi-repo tabs. **The read half has landed**: `GitBackend::worktrees` lists every checkout, the main one first — gitoxide counts only the linked ones — with each one's `HEAD`, whether it is the one hideGit was opened from, any lock reason, and whether its directory is gone. `HEAD` is the point: a branch checked out in one worktree cannot be checked out in another, and nothing else makes that rule visible. **Read and shown**: the sidebar has a `WORKTREES` section, present only when there is more than one checkout, with the branch each holds. `add`, `remove` and `prune` are on the CLI side with the rest of the writes. `add` fails rather than choosing for the user when the branch is already checked out elsewhere — that refusal is the one rule worktrees have, working. `remove` refuses a checkout with uncommitted work until asked again with force. `prune` leaves locked worktrees alone, which git guarantees and no flag overrides. The interface calls all three: the worktree row offers `remove` and `prune`, and the branch row offers "Check out in a new worktree…", each gated so it never offers an action git would refuse — including the checkout of a branch another worktree already holds. |
| **Git LFS** | Largely inherited from shelling out to `git`. **Pointer files have landed**: hideGit reads the object database through gitoxide, so what it sees for an LFS-tracked file is always the pointer — whether or not `git-lfs` is installed and whether or not the object was fetched. A diff of one now reports the size it stands for rather than three lines of `oid sha256:…`, including when a file moves into or out of LFS. Recognising a pointer needs no `git-lfs`, so nothing here depends on it being installed. hideGit also **says when a repository needs `git-lfs` and it is not installed** — the failure users actually hit, because an LFS repository checked out without the tool has pointer text where its content should be and that reads as corruption. Detected by reading the attribute files rather than by asking the tool, so the answer is the same either way. **Fetch state has landed**: a diff of an LFS file says when the object is *not* in the local store — the state a clone leaves every object in, where the file on disk is pointer text that reads as corruption rather than as an absence. The store is `<storage>/objects/<oid[0..2]>/<oid[2..4]>/<oid>`, with `<storage>` honouring `lfs.storage` (relative to `.git`, or absolute) and defaulting to `.git/lfs`. That layout was **verified against `git-lfs` 3.7.1** rather than assumed, and one `#[ignore]`d test re-checks it against the real tool so the claim cannot rot quietly; the rest of the suite builds the store by hand, so CI still needs no `git-lfs` on any platform. A present file is taken as a present object — verifying it would mean hashing every large file to draw a diff header, which is the cost the pointer exists to avoid. |
| **Interactive rebase editor** | A richer one than M6's: a plan the graph itself can be dragged into, and `--autosquash`. **`--autosquash` has landed**: `RebasePlan` is now an enum — ordinary, autosquash, or hideGit's own steps — because the last two are mutually exclusive by construction and a struct with a list and a flag could hold both. Git computes the reordering and hideGit accepts its todo unedited, which is what [ADR-0007](./adr/0007-rebase-plan-through-the-environment.md) asks for. **Plan rows are now dragged to reorder**, remove-and-insert rather than swapping, with the one-step buttons kept for the keyboard. Dragging a commit *from the graph* into the plan is a different thing and is **not** planned as stated: the plan editor is a modal scrim over the whole screen — which is what makes it abandonable — so the graph is covered while it is open. Putting the two on screen at once is a layout decision, not an implementation detail. |
| **Internationalisation** | **The scaffolding has landed** ([ADR-0008](./adr/0008-translations-as-toml-catalogues.md)): keys in the code, strings in a TOML catalogue, no i18n runtime. English is `locales/en.toml`, compiled in; a translation is the same file in a `locales` directory beside `config.toml`. Proven on the settings panel — the other ~690 strings are still inline literals, and moving them is the work that remains. **PT-BR has landed**, compiled in rather than left as a file to copy, covering every key the catalogue currently has. The language comes from the environment — `LC_ALL`, `LC_MESSAGES`, `LANG` — the way every other application on the machine decides it; there is no setting, because an application that disagrees with the system about its own language is a bug report nobody can reproduce. The other ~690 inline literals are still the work that remains, and every one of them moved into the catalogue is a string PT-BR gets for free. |
