...
| **Worktrees** | Growing in use; fits naturally alongside multi-repo tabs. **The read half has landed**: `GitBackend::worktrees` lists every checkout, the main one first — gitoxide counts only the linked ones — with each one's `HEAD`, whether it is the one hideGit was opened from, any lock reason, and whether its directory is gone. `HEAD` is the point: a branch checked out in one worktree cannot be checked out in another, and nothing else makes that rule visible. Nothing shows it yet, and nothing writes: `add`, `remove` and `prune` are still to come. |
